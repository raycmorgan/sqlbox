 ...
 */
function save(box, obj, mainCallback) {
  if (!mainCallback) {
  ...
  // FIXME: arrays are ref copied still, so they will be mutated
  obj = _.clone(obj);

  async.series({
    beginTxn: _.bind(box.client.query, box.client, 'BEGIN'),
    beforeSave: _.partial(runHook, box, obj, 'beforeSave'),
    validation: _.partial(validate, box, obj),
...
function modify(box, id, opts, mutator, callback) {
  ...
  function attemptModify(tryNumber) {
    ...
    if (tryNumber > (opts.maxRetries || 3)) {
      return callback(new BoxError(504, 'Maximum number of retries hit.'));
    }
    ...
    box.get(id, opts, function modifyGet_(err, obj) {
      ...
      if (!ensuresPass) {
        return callback(new BoxError(409, 'Ensure condition failed.'));
      }
...
function saveNew(box, obj, callback) {
  ...
  box.client.query(query.toQuery(), function saveNew_(err, result) {
    ...
    if (result.rows.length) {
      runHooks(box, result.rows[0], ['afterCreate', 'afterSave'], function (err) {
        if (err) {
          return callback(err);
        }
...
function saveUpdate(box, obj, callback) {
  ...
  box.client.query(query, function saveUpdate_(err, result) {
    ...
    if (result.rows.length) {
      runHooks(box, result.rows[0], ['afterUpdate', 'afterSave'], function (err) {
        if (err) {
          return callback(err);
        }
        callback(null, build(box, result.rows[0]));
      ...
      var msg =
        ('Row with id ' + id + ' was not found in ' + box.name + ', ' +
