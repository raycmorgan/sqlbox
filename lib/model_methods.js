...
function saveNew(box, obj, callback) {
  ...
  box.client.query(query.toQuery(), function saveNew_(err, result) {
    ...
    if (result.rows.length) {
      if (typeof box.hooks.afterCreate === 'function') {
        box.hooks.afterCreate(result.rows[0]);
      }

      if (typeof box.hooks.afterSave === 'function') {
        box.hooks.afterSave(result.rows[0]);
      }
...
function saveUpdate(box, obj, callback) {
  ...
  box.client.query(query, function saveUpdate_(err, result) {
    ...
    if (result.rows.length) {
      if (typeof box.hooks.afterUpdate === 'function') {
        box.hooks.afterUpdate(result.rows[0]);
      }
