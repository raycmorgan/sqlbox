...
function build(box, obj, callback) {
  ...
  runHooks(box, instance, ['afterFetch'], callback);
}
...
function all(box, properties, opts, callback) {
  ...
  var t = box.table;
  var sql = t.select(t.star());

  sql = whereClause(box, sql, properties);
  ...
  if (opts.order) {
    var ordering = _.map(opts.order, function (v, k) {
      return box.table[columnSource(box, k)][v];
    });
...
function saveNew(box, obj, callback) {
  ...
  var query = dialect.getSaveQuery(box, obj);
  ...
  box.client.query(query.text, query.values, function saveNew_(err, result) {
    ...
    if (dialect.getSavedRowCount(result)) {
      build(box, dialect.getSavedRecord(result), function (err, record) {
        if (err) {
          return callback(err);
        }

        runHooks(box, record, ['afterCreate', 'afterSave'], callback);
      });
    } else {
...
function saveUpdate(box, obj, where, callback) {
  ...
  var query = dialect.getUpdateQuery(box, where, whereClause, sourceObject);
  ...
  box.client.query(query.text, query.values, function saveUpdate_(err, result) {
    ...
    if (dialect.getSavedRowCount(result)) {
      build(box, dialect.getSavedRecord(result), function (err, record) {
        ...
        runHooks(box, record, ['afterUpdate', 'afterSave'], callback);
      });
    } else {
      var msg =
