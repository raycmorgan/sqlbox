...
function saveNew(box, obj, callback) {
  ...
  box.client.query(query.toQuery(), function saveNew_(err, result) {
    ...
    if (result.rows.length) {
      runHooks(box, obj, ['afterCreate', 'afterSave']);
...
function saveUpdate(box, obj, callback) {
  ...
  box.client.query(query, function saveUpdate_(err, result) {
    ...
    if (result.rows.length) {
      runHooks(box, obj, ['afterUpdate', 'afterSave']);
      callback(null, build(box, result.rows[0]));
    } else {
      var msg = 
...
function runHook(box, obj, hookName) {
  var hook = box.hooks[hookName];

  if (_.isFunction(hook)) {
    hook(obj);
  } else if (_.isArray(hook)) {
    _.each(hook, function (fn) {
      if (_.isFunction(fn)) {
        fn(obj);
      }
    });
...
function runHooks(box, obj, hooks) {
  _.each(hooks, _.partial(runHook, box, obj));
}
