...
function mget(box, ids, opts, callback) {
  ...
  box.all({id: {in: _.map(ids, Number)}}, opts, callback);
}
...
function all(box, properties, opts, callback) {
  ...
  if (opts.includes) {
    includes = relations.includeQuery(box, opts.includes);
    query.from(includes.query);

    delete opts.limit;
  } else {
    query.from(t.as(pluralize(box.name)));
  }
  ...
  if (typeof opts.order === 'function') {
    query.order(opts.order(box.table.as(pluralize(box.name))));
  }
  ...
  var queryClientForm = query.toQuery();

  if (includes) {
    queryClientForm.rowMode = 'array';
  }

  box.client.query(queryClientForm, function all_(err, result) {
    if (err) {
      return callback(err);
    }

    if (!includes) {
      callback(null, _.map(result.rows, box.build));
    } else {
      try {
        var mapIndex = 0;
        var idColumns = [];
        ...
        _.each(result.fields, function (field, i, fields) {
          var previousField = fields[i-1];
          if (previousField && previousField.tableID !== field.tableID) {
            mapIndex++;
            columnRanges.push(currentRange.concat(i-1));
            currentRange = [i];
          }

          field.tableName = includes.tableMap[mapIndex].name;
        ...
        columnRanges.push(currentRange.concat(result.fields.length - 1));

        var res = [];
        var pathRelationMap = _.object(_.zip(_.pluck(includes.tableMap, 'name'), _.pluck(includes.tableMap, 'relation')));
        var pathModelMap = _.object(_.zip(_.pluck(includes.tableMap, 'name'), _.pluck(includes.tableMap, 'model')));

        _.each(result.rows, function (row) {
          var ids = _.map(idColumns, function (idIndex) {
            return row[idIndex];
          });

          var pathIdMap = _.object(_.zip(_.pluck(includes.tableMap, 'name'), ids));

          _.each(includes.tableMap, function (table, i, tableMap) {
            var record;
            var parentsCollection;
            var rowId = row[idColumns[i]];
            ...
            if (i === 0) {
              ...
              for (var j = 1; j < path.length; j++) {
                if (_.isArray(grandparent)) {
                  grandparent = _.findWhere(grandparent, {id: pathIdMap[subpath]});
                }

                subpath += '.' + path[j];

                var relation = pathRelationMap[subpath];

                if (relation === 'hasMany') {
                  grandparent = grandparent[path[j]] = grandparent[path[j]] || [];
                }
              ...
              parentsCollection = grandparent;
            }

            if (_.isArray(parentsCollection)) {
              record = _.findWhere(parentsCollection, {id: rowId});
            } else {
              record = parentsCollection;
            }
            
            if (!record || _.size(record) === 0) {
              if (!record) {
                record = {};
              }

              for (var ii = columnRanges[i][0]; ii <= columnRanges[i][1]; ii++) {
                var field = result.fields[ii];
                record[field.name] = row[ii];
              }

              record = build(pathModelMap[table.name], record);

              if (_.isArray(parentsCollection)) {
                parentsCollection.push(record);
              }
            }
          });
        });
...
function saveNew(box, obj, callback) {
  ...
  box.client.query(query.toQuery(), function saveNew_(err, result) {
    ...
    if (result.rows.length) {
      ...
      callback(new BoxError(500, 'Database returned no error, but no row was returned.'));
    }
  });
}
...
function saveUpdate(box, obj, where, callback) {
  ...
  var t = box.table.as(pluralize(box.name));
