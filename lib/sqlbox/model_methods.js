...
function all(box, properties, opts, callback) {
  ...
  if (opts.includes) {
    ...
    _.each(includes.order, function (prop) {
      query.select('"' + prop.name + '".*');
    });

    // includes.order.push({model: box, name: box.tableName});
  ...
  box.client.query(queryClientForm, function all_(err, result) {
    ...
    if (!includes) {
      ...
      // _.object(
      //   _.zip(
      //     ['*'].concat(_.pluck(includes.order, 'name')),
      //     _.values(_.groupBy(result.fields, 'tableID'))
      //   )
      // );


      // console.log(
      //   _.reduce(rows, function (memo, row) {
      //     var obj;

      //     _.each(relationFields, function (fields, relationName) {
      ...
      _.each(result.rows, function (row) {
        ...
        _.each(row, function (column, i) {
          var field = result.fields[i];
          var includeName = null;

          tableId = tableId || field.tableID;

          // Moved to next joined table
          if (tableId !== field.tableID) {
            tableIndex++;
            tableId = field.tableID;

            var prevRelation = obj[includes.order[tableIndex].name];

            if (prevRelation && !_.isArray(prevRelation)) {
              obj[includes.order[tableIndex].name] = [prevRelation, {}];
            } else if (_.isArray(prevRelation)) {
              obj[includes.order[tableIndex].name].push({});
            } else {
              obj[includes.order[tableIndex].name] = {};
            }
          }

          if (tableIndex >= 0) {
            includeName = includes.order[tableIndex].name;
          }

          if (!includeName) {
            obj[field.name] = column;

            if (field.name === 'id') {
              var prevObj = _.findWhere(res, {id: column});
              if (prevObj) {
                obj = prevObj;
              }
            }
          } else {
            if (_.isArray(obj[includeName])) {
              _.last(obj[includeName])[field.name] = column;
            } else if (_.isObject(obj[includeName])) {
              obj[includeName][field.name] = column;
            }
          }
        });

        if (!_.contains(res, obj)) {
          res.push(obj);
        }
      });
